...
public class RejectJobApplicationConsumer(
...
) : IMessageConsumerPort<RejectJobApplicationMessage>
{
    public async Task ConsumeAsync(RejectJobApplicationMessage message, CancellationToken cancellationToken = default)
    {
        ...
        var notification = new Notification(
            profile.UserId,
            $"Tu postulación al empleo \"{message.JobTitle}\" en \"{message.CompanyName}\" ha sido rechazada.",
        ...
        if (message.Channels.Contains(NotificationChannel.Email))
        {
            var html = htmlTemplatePort.Render(
                HtmlTemplate.CandidateRejected,
                [
            ...
            && !string.IsNullOrWhiteSpace(profile.PhoneNumber))
        {
            sent |= await notificationPort.SendAsync(
                ...
                "La empresa ha decidido continuar con otros candidatos en esta ocasión.\n\n" +
                "Esta decisión corresponde únicamente a este proceso de selección y no define " +
                "tus capacidades ni tu potencial profesional.\n\n" +
                "🔎 Te recomendamos continuar explorando nuevas oportunidades que se ajusten a tu perfil.\n\n" +
